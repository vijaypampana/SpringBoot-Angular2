...
class ShipwreckStub {
	...
	static List<Shipwreck> list() {
		return new ArrayList<Shipwreck>(wrecks.values())
	}

	static Shipwreck create(Shipwreck wreck) {
		idIndex += idIndex
		wreck.setId(idIndex)
	...
	static Shipwreck get(Long id) {
		return wrecks.get(id)
	}

	static Shipwreck update(Long id, Shipwreck wreck) {
		wrecks.put(id, wreck)
		return wreck
	}

	static Shipwreck delete(Long id) {
